...
class FoundationPose:
  ...
  def register_xyz(self, K, rgb, depth, ob_mask, ob_mask_file, ob_id=None, glctx=None, iteration=5):
    '''Copmute pose from given pts to self.pcd
    @pts: (N,3) np array, downsampled scene points
    '''
    set_seed(0)
    ...
    scores, vis = self.scorer.predict(mesh=self.mesh, rgb=rgb, depth=depth, K=K, 
                                      ...
                                      mesh_diameter=self.diameter, mask=ob_mask, get_vis=True)
    if vis is not None:
      parts = ob_mask_file.split('/')
      object_id = parts[-3]
      mask_id = os.path.basename(ob_mask_file).split('.')[0]
      save_dir = os.path.join(f'{self.debug_dir}/xyz_sam6d/{object_id}') ###
  ...
  def register_ipd(self, K, rgb, depth, ob_mask, ob_mask_file, ob_id=None, glctx=None, iteration=5):
    '''Copmute pose from given pts to self.pcd
    @pts: (N,3) np array, downsampled scene points
    '''
    set_seed(0)
    ...
    if vis is not None:
      ...
      object_id = parts[-3]
      mask_id = os.path.basename(ob_mask_file).split('.')[0]
      save_dir = os.path.join(f'{self.debug_dir}/ipd_sam6d/{object_id}') ###
      os.makedirs(save_dir, exist_ok=True)
      imageio.imwrite(f'{save_dir}/{mask_id}.png', vis)
